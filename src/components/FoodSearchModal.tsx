...
export default function FoodSearchModal({
  ...
  // Search food catalog — uses apiFetch for automatic auth handling
  ...
  // Log food to diary — uses apiFetch for automatic auth handling
  const handleLogFood = useCallback(async () => {
    ...
    try {
      const res = await apiFetch(`/api/v2/nutrition/log`, {
        ...
        body: JSON.stringify({
          logged_at: new Date(`${selectedDate}T12:00:00Z`).toISOString(),
          meal_type: mealType,
  ...
  // Lookup barcode via backend proxy — uses apiFetch
  const lookupBarcode = useCallback(
    async (code: string) => {
      setBarcodeLoading(true);
      try {
        const res = await apiFetch(
          `/api/v2/nutrition/barcode/${encodeURIComponent(code)}`
        );
            ...
            {selectedFood ? (
              ...
              >
                <div className="rounded-xl bg-white/[0.03] border border-white/5 p-4">
                  ...
                  {selectedFood.brand && (
                    <p className="text-[10px] text-gray-500">{selectedFood.brand}</p>
                  )}
                  <p className="text-[10px] text-gray-600 mt-1">
                    Per 100g: {selectedFood.calories_per_100g} kcal •
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 rounded-xl bg-white/5 border border-white/10 py-3 text-xs font-medium text-gray-400 hover:text-white transition-colors"
                  >
                    Back
                  </button>
                  <button
                    ...
                    disabled={isLogging}
                    className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-accent-indigo via-accent-purple to-accent-cyan py-3 text-xs font-semibold text-white disabled:opacity-50 shadow-[0_0_10px_rgba(168,85,247,0.3)]"
                  >
                    {isLogging ? (
              ...
              <div className="space-y-3">
                <div className="relative">
                  ...
                  <input
                    ...
                    autoFocus
                    className="w-full rounded-xl bg-white/5 border border-white/10 pl-10 pr-4 py-3 text-sm text-white placeholder-gray-600 outline-none focus:border-accent-purple focus:ring-1 focus:ring-accent-purple/50"
                  />
                  {isSearching && (
                ...
                <div className="space-y-1.5 max-h-[40dvh] overflow-y-auto transform-gpu">
                  {searchResults.map((food) => (
                    <button
                      ...
                      className="w-full flex items-center gap-3 rounded-xl bg-white/[0.02] border border-white/[0.04] px-3 py-2.5 text-left hover:bg-white/[0.05] transition-colors"
                    >
                ...
                <button
                  ...
                  className="w-full text-center py-3 text-[11px] font-medium text-accent-purple hover:text-accent-cyan transition-colors"
                >
                ...
                {cameraAvailable ? (
                  <div className="space-y-3">
                    <div
                      id="barcode-reader"
                      ref={scannerRef}
                      className="w-full h-48 rounded-xl overflow-hidden bg-black/50 border border-white/10"
                    />
                    {isScannerActive && (
                  ...
                  <div className="rounded-xl bg-white/[0.03] border border-white/5 p-4 text-center space-y-3">
                    ...
                    <p className="text-xs text-gray-400">
                      Camera unavailable. Type barcode manually:
                    </p>
                    <div className="flex gap-2">
                      <input
                        type="text"
                        ...
                        className="flex-1 rounded-xl bg-white/5 border border-white/10 px-4 py-2.5 text-sm text-white placeholder-gray-600 outline-none focus:border-accent-purple"
                      />
                      <button
                        onClick={handleManualBarcode}
                        disabled={barcodeValue.length < 5 || barcodeLoading}
                        className="rounded-xl bg-accent-purple/20 border border-accent-purple/30 px-4 py-2.5 text-xs font-medium text-accent-purple disabled:opacity-50"
                      >
                        {barcodeLoading ? (
