...
/**
 * LogFoodDrawer — V2.5.2 Multi-Measure Food Logging Interface
 *
 * Features:
 ...
 * - 0ms real-time macro preview (values clamped to 1 decimal place)
 * - Optimistic submission via Zustand sync store
 * - Phase 4: Allergen safety interceptor with confirmation interstitial
 */
...
// ---------------------------------------------------------------------------
// AllergenConfirmModal — inline interstitial overlay
// ---------------------------------------------------------------------------

interface AllergenConfirmModalProps {
  isOpen: boolean;
...
function AllergenConfirmModal({
  isOpen,
  overlappingAllergens,
  onCancel,
  onConfirm,
}: AllergenConfirmModalProps) {
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div
        ...
        >
          <motion.div
            ...
            className="w-full max-w-sm rounded-2xl border border-[#E11D48]/30 bg-[#0d0305] p-6 shadow-[0_0_60px_rgba(225,29,72,0.2)]"
          >
            {/* Icon + Title */}
            <div className="flex items-center gap-3 mb-4">
              <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#E11D48]/10 border border-[#E11D48]/20 flex-shrink-0">
                <AlertTriangle className="h-5 w-5 text-[#E11D48]" />
              </div>
              <h3 className="text-sm font-bold text-white">
                Confirm Allergen Exposure
              </h3>
            </div>

            {/* Body */}
            <p className="text-[13px] text-gray-400 leading-relaxed mb-2">
              Logging this item contains allergens that may represent a health
              risk. Do you wish to override and log?
            </p>
            <div className="flex flex-wrap gap-1.5 mb-5">
              {overlappingAllergens.map((a) => (
                <span
                  key={a}
                  className="text-[10px] font-bold uppercase tracking-wide px-2 py-0.5 rounded-full bg-[#E11D48]/15 text-[#E11D48] border border-[#E11D48]/25"
                >
                  {a}
                </span>
              ))}
            ...
            <div className="flex gap-3">
              <button
                onClick={onCancel}
                className="flex-1 rounded-xl border border-white/10 bg-white/5 py-2.5 text-xs font-semibold text-gray-300 hover:bg-white/10 transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={onConfirm}
                className="flex-1 rounded-xl bg-[#E11D48] py-2.5 text-xs font-bold text-white hover:bg-rose-700 transition-colors shadow-[0_0_20px_rgba(225,29,72,0.35)]"
              >
                Yes, Log Anyway
              </button>
            </div>
...
export default function LogFoodDrawer({
  ...
  const userAllergies = useUserStore((s) => s.allergies);
  ...
  // ---------------------------------------------------------------------------
  // Phase 4: Allergen intersection — case-insensitive, store already normalizes
  // ---------------------------------------------------------------------------
  const overlappingAllergens = useMemo<string[]>(() => {
    const foodAllergens = food.allergens ?? [];
    const profileAllergies = userAllergies ?? [];
    if (foodAllergens.length === 0 || profileAllergies.length === 0) return [];
    const normalizedProfile = new Set(profileAllergies.map((a) => a.toLowerCase().trim()));
    return foodAllergens.filter((a) => normalizedProfile.has(a.toLowerCase().trim()));
  }, [food.allergens, userAllergies]);

  const hasAllergenWarning = overlappingAllergens.length > 0;
  ...
  // 0ms real-time macro preview — values clamped to 1 d.p.
  const previewMacros = useMemo(() => {
    ...
    return {
      calculated_calories: Number(macros.calculated_calories).toFixed(1),
      calculated_protein: Number(macros.calculated_protein).toFixed(1),
      calculated_carbs: Number(macros.calculated_carbs).toFixed(1),
      calculated_fat: Number(macros.calculated_fat).toFixed(1),
      baseQty,
    };
  ...
  // ---------------------------------------------------------------------------
  // Core submit — dispatches optimistic log. Allergen check gates this.
  // ---------------------------------------------------------------------------
  const executeSubmit = useCallback(
    async (allergenOverride: boolean) => {
  ...
  // CTA click handler — intercepts if allergen warning is active
  ...
  }, [hasAllergenWarning, isValid, executeSubmit]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <>
          ...
          >
            ...
            >
              ...
              <div className="px-5 pb-6 space-y-4">
                ...
                <div className="flex gap-3 items-end">
                  ...
                  <div className="flex-[1.5] flex flex-col gap-1.5 relative" ref={measureRef}>
                    ...
                    >
                      ...
                      <ChevronDown
                        className={`h-4 w-4 text-zinc-500 transition-transform ${measureOpen ? "rotate-180" : ""}`}
                      />
                    </button>
                          ...
                          {food.measures.map((measure) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                {isActive && <Check className="h-3.5 w-3.5 text-cyan-400 flex-shrink-0" />}
                              </button>
                            );
                          })}
                        </motion.div>
                ...
                {previewMacros.baseQty > 0 &&
                  selectedMeasure &&
                  selectedMeasure.measure_name !== food.base_unit && (
                    <p className="text-[10px] text-gray-500">
                      = {Number(previewMacros.baseQty).toFixed(1)} {food.base_unit}
                    </p>
                  )}
                ...
                {/* ── Phase 4: Allergen Warning Banner ── */}
                {hasAllergenWarning && (
                  <div className="bg-[#E11D48]/10 border border-[#E11D48]/30 rounded-button p-4 flex items-start gap-3 my-3 animate-pulse">
                    <AlertTriangle className="h-5 w-5 text-[#E11D48] flex-shrink-0 mt-0.5" />
                    <p className="text-xs text-[#E11D48] leading-relaxed font-medium">
                      ⚠️ ALLERGEN WARNING: This item contains allergens registered to
                      your profile:{" "}
                      <span className="font-bold uppercase">
                        {overlappingAllergens.join(", ")}.
                      </span>
                    </p>
                  </div>
                )}

                {/* Submit Button — crimson when allergen warning is active */}
                <button
                  onClick={handleSubmit}
                  disabled={!isValid || isSubmitting}
                  className={`w-full flex items-center justify-center gap-2 rounded-xl py-3.5 text-sm font-semibold text-white disabled:opacity-40 transition-all active:scale-[0.98] ${
                    hasAllergenWarning
                      ? "bg-status-rose hover:bg-rose-700 shadow-[0_0_20px_rgba(225,29,72,0.3)] hover:shadow-[0_0_30px_rgba(225,29,72,0.5)]"
                      : "bg-gradient-to-r from-indigo-500 via-purple-500 to-cyan-500 shadow-[0_0_20px_rgba(168,85,247,0.25)] hover:shadow-[0_0_30px_rgba(168,85,247,0.4)]"
                  }`}
                >
                  {isSubmitting ? (
      ...
      {/* ── Phase 4: Allergen Confirmation Interstitial ── */}
      <AllergenConfirmModal
        isOpen={showAllergenModal}
        overlappingAllergens={overlappingAllergens}
